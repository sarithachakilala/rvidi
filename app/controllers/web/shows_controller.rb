module Web
  ...
  class ShowsController < Web::BaseController
    ...
    def destroy
      ...
      respond_to do |format|
        format.html { redirect_to web_shows_url }
        format.json { head :no_content }
      end
    end
    ...
    def play_cameo
      ...
      respond_to do |format|
        format.html { redirect_to web_show_url(@cameo.show_id, :cameo_id => @cameo.id) }
        format.js {}
        format.json { head :no_content }
      end
    end
    ...
    def invite_friend_toshow
      ...
      notification.save!
      redirect_to edit_web_show_path(:id=>@show.id), :notice => "Invitatiion sent successfully"
    end
    ...
    def check_password
      ...
      if @show.display_preferences_password == params[:password]
        ...
        redirect_to web_show_path(:id => @show.permalink, :preference => @display_prefernce)
      else
        redirect_to web_show_path(:id => @show.permalink), :notice => "Invalid Password: Please enter the correct password! "
      end
    end
    ...
    def status_update
      ...
      if params[:status] == "end"
        ...
        redirect_to edit_web_show_path(@show.id),
          :notice => "Successfully Show got #{params[:status]}ed."
      else
        redirect_to web_show_path(:id=> @show.id ),
          :notice => "Successfully Show got #{params[:status]}ed."
      end
    end
